/*****************************************************************************//**
  * @file   sd_hal_mpu6050.c
  * @author  Sina Darvishi
  * @date     Feb 19, 2016
  * @brief 	 MPU6050 API configuration. Slightly adapted by M'barek ZACRI
  * to include an initialisation phase that incorporate sensors offsets
...
void mpuInit(SD_MPU6050 *mpu,I2C_HandleTypeDef* hi2c){
	  SD_MPU6050_Result result ;
	  result = SD_MPU6050_Init(hi2c,mpu,SD_MPU6050_Device_0,SD_MPU6050_Accelerometer_2G,SD_MPU6050_Gyroscope_250s );
	  HAL_Delay(1000);
	  ...
	  while(init<cnt){
		  init++;
		  SD_MPU6050_ReadGyroscope(hi2c,mpu);
		  SD_MPU6050_ReadAccelerometer(hi2c,mpu);
		  ax += mpu->Gyroscope_X;
		  ay += mpu->Gyroscope_Y;
		  az += mpu->Gyroscope_Z;
		  gx += mpu->Accelerometer_X;
		  gy += mpu->Accelerometer_Y;
		  gz += mpu->Accelerometer_Z;
	  }
	  mpu->init_Gyroscope_X = (int16_t)(ax/cnt);
	  mpu->init_Gyroscope_Y = (int16_t)(ay/cnt);
	  mpu->init_Gyroscope_Z = (int16_t)(az/cnt);
	  mpu->init_Accelerometer_X = (int16_t)(gx/cnt);
	  mpu->init_Accelerometer_Y = (int16_t)(gy/cnt);
	  mpu->init_Accelerometer_Z = (int16_t)(gz/cnt +(1/mpu->Acce_Mult));
  }
...
SD_MPU6050_Result SD_MPU6050_Init(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, SD_MPU6050_Device DeviceNumber, SD_MPU6050_Accelerometer AccelerometerSensitivity, SD_MPU6050_Gyroscope GyroscopeSensitivity)
{
	...
	//------------------
		...
		d[1] = 0x03;   //PLL with Z axis gyroscope reference
...
SD_MPU6050_Result SD_MPU6050_ReadAccelerometer(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	/* Format */
	DataStruct->Accelerometer_X = (-(int16_t)(data[0] << 8 | data[1]) - DataStruct->init_Accelerometer_X);
	DataStruct->Accelerometer_Y = (-(int16_t)(data[2] << 8 | data[3]) - DataStruct->init_Accelerometer_Y);
	DataStruct->Accelerometer_Z = ((int16_t)(data[4] << 8 | data[5]) - DataStruct->init_Accelerometer_Z);
...
SD_MPU6050_Result SD_MPU6050_ReadGyroscope(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	/* Format */
	DataStruct->Gyroscope_X = (-(int16_t)(data[0] << 8 | data[1]) - DataStruct->init_Gyroscope_X);
	DataStruct->Gyroscope_Y = (-(int16_t)(data[2] << 8 | data[3]) - DataStruct->init_Gyroscope_Y);
	DataStruct->Gyroscope_Z = ((int16_t)(data[4] << 8 | data[5]) - DataStruct->init_Gyroscope_Z);
